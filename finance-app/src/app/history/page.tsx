  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 sm:p-8">
      <header className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between mb-8">
        <TabNav active="history" onSettingsClick={() => setShowSettingsModal(true)} />
        <SignOutButton />
      </header>
      ...
      <section className="mb-8">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold text-slate-900">{periodLabel}</h1>
          </div>
          <PeriodSelector value={period} onChange={setPeriod} today={today} />
        </div>
      </section>

      <section className="grid gap-6 lg:grid-cols-2 mb-8">
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
          <p className="text-sm uppercase tracking-[0.25em] text-slate-500">Starting / Ending Balance</p>
          {balance === null ? (
            ...
            <div className="mt-5 grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                {balance.ending_balance === null ? (
                  <div className="mt-1 inline-flex rounded-3xl bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-700">
                    Not yet available
                  </div>
                ) : (
                  <p className="mt-1 text-3xl font-bold text-slate-900">
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
          <p className="text-sm uppercase tracking-[0.25em] text-slate-500">Total Income</p>
          <p className="mt-5 text-3xl font-bold text-slate-900">{formatCurrency(totalIncome)}</p>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-900">{periodLabel}</h2>
          </div>
          <button
            type="button"
